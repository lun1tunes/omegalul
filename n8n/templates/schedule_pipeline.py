...
if((obj(req.schema_catalogue)||obj(req.rag_evidence?.schema_catalogue))&&!irEvents.length&&changes.length&&!useTimelineCommissioning)findings.push({code:'TYPED_IR_REQUIRED_FOR_CATALOGUE_RENDER',severity:'error'});
...
const sourceCoverage=useTimelineCommissioning?100:(nonKeep.length?Math.min(100,Math.round(100*sourceMap.length/nonKeep.length)):100);
const scopeFit=useTimelineCommissioning?100:(scope.length?Math.round(100*scope.filter(k=>represented.has(k)).length/scope.length):0);
const evidenceCompleteness=Math.round(.6*requirementCoverage+.4*sourceCoverage);
const sourceAuthority=useTimelineCommissioning?100:(scope.length?Math.round(100*scope.filter(k=>citedKeywords.has(k)).length/scope.length):0);
...
const timelinePath=intake.build_mode==='REVISE'&&(wellFacts.length>0||arr(b.source_map)&&b.source_map.some(s=>clean(s.path)==='timeline_commissioning_revise'));
