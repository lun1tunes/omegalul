...
const generated=String(m.generated_schedule||'').slice(0,200000);
...
const normalizedFindings=findings.length?findings:(Array.isArray(root.packet_findings)?root.packet_findings:[]),reasonCodes=normalizedFindings.map(f=>String(f.code||'SCHEDULE_PIPELINE_GATE')),suppliedQuestions=Array.isArray(x.questions)?x.questions.filter(q=>q&&typeof q==='object').slice(0,100):[];
const questions=gaps.length?gaps.map((g,i)=>({id:`schedule_gap_${i+1}`,question:String(g.question||`${g.keyword||'SCHEDULE'} ${g.entity||''} ${g.field||''}: ${g.reason||'required evidence is missing'}`).trim(),expected_format:String(g.expected_format||'value with units and provenance'),required:true})):suppliedQuestions.length?suppliedQuestions:normalizedFindings.map((f,i)=>({id:`schedule_finding_${i+1}`,question:String(f.message||f.code||'Resolve the SCHEDULE pipeline finding.'),required:true}));
