...
def test_agent_app_routes(tmp_path: Path) -> None:
    ...
    assert health["agent_id"] == "demo"
    assert health["tools"] == ["echo"]
    assert health["mas_version"] == (ROOT / "VERSION").read_text(encoding="utf-8").splitlines()[0].strip()
    assert isinstance(health.get("env_files"), list)
