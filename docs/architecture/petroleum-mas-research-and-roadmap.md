...
| Commissioning REVISE timeline | `schedule_timeline_runtime.py`: `unlisted_wells_policy` remove **только** через typed enum (`options`/`req`/HITL); prose «убери…» → `needs_input` (`UNLISTED_WELLS_POLICY_REQUIRED`); new-well HITL (`schedule-builder-hitl-attachment-v1`); combat 0–3 | live Orch→Builder E2E с LLM/RAG на корпоративных Excel |
