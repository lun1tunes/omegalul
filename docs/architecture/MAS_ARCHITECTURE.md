...
graph TD
    ...
    subgraph "SCHEDULE Subsystem"
        S_Intake["Schedule Intake<br/><small><i>tnavigator-schedule-intake</i></small>"]
        O --> |Route| S_Intake
        
        subgraph "Baseline Analysis"
            S_B_Ana["Baseline Analyzer<br/><small><i>tnavigator-schedule-baseline-analyzer</i></small>"]
            S_B_Dec["Baseline Decoder<br/><small><i>tnavigator-schedule-baseline-decoder</i></small>"]
            S_B_Qry["Baseline Query<br/><small><i>tnavigator-schedule-baseline-query</i></small>"]
            S_Intake --> S_B_Ana --> S_B_Dec --> S_B_Qry
        end
        ...
        subgraph "Generation Pipeline"
            S_Plan["Schedule Planner<br/><small><i>tnavigator-schedule-planner</i></small><br/>[LLM REQUIRED]"]
            S_Bld["Schedule Builder<br/><small><i>tnavigator-schedule-builder</i></small><br/>[LLM REQUIRED]"]
            S_Rnd["Schedule Renderer<br/><small><i>tnavigator-schedule-renderer</i></small>"]
        ...
        subgraph "Validation & Release"
            S_Val["Schedule Validator<br/><small><i>tnavigator-schedule-validator</i></small>"]
            S_Ver["Schedule Verifier<br/><small><i>tnavigator-schedule-verifier</i></small><br/>[LLM REQUIRED]"]
            S_Mrg["Schedule Merge<br/><small><i>tnavigator-schedule-merge</i></small>"]
            S_Rel["Schedule Release<br/><small><i>tnavigator-schedule-release</i></small>"]
            
            S_Rnd --> S_Val --> S_Ver --> S_Mrg --> S_Rel
        end
    end
```
...
A highly specialized RAG-driven code generation pipeline.
*   **Knowledge Ingestion:** `tnavigator-schedule-knowledge-ingestion` / `hybrid-retrieval` parse tNav manuals and baseline `.SCH` files into `pgvector`.
*   **Builder Pipeline:** 
    *   `tnavigator-schedule-planner`: Defines the strategy for the schedule.
    *   `tnavigator-schedule-builder`: Writes the actual tNavigator syntax.
    *   `tnavigator-schedule-validator` & `tnavigator-schedule-verifier`: Syntax and logic checks against tNav rules.
    *   `tnavigator-schedule-merge`: Integrates the generated code with the baseline.
...
**SCHEDULE Subsystem:**
- `tnavigator-schedule-intake`
- `tnavigator-schedule-baseline-analyzer` / `decoder` / `query`
- `tnavigator-schedule-knowledge-ingestion` / `hybrid-retrieval`
- `tnavigator-schedule-planner` / `builder` / `renderer`
- `tnavigator-schedule-validator` / `verifier` / `merge` / `release`
...
*Note: After importing these, go to the workflow settings in n8n and copy the internal `Workflow IDs` for all the top-level Adapters (Excel, Schedule Intake, Calculation). You will need them for Step 2.*

### Step 2: The Orchestrator (Import Second)
1. Import `universal-engineering-orchestrator.workflow.json`.
2. Open the Orchestrator in the UI.
3. Locate the `Execute Workflow` node named **"Route invocation action"**.
4. Inside this node, replace the placeholder IDs (e.g., `REPLACE_EXCEL_ADAPTER_IN_UI`) with the actual `Workflow IDs` you copied in Step 1.

### Step 3: Global Variables Setup (n8n UI)
Go to **Settings -> Variables** in your n8n UI and configure the environment for the FastAPI Microservices:
- **`EXCEL_TOOLS_URL`**: `http://<your-windows-local-ip>:8080` (Points to `excel-agent-tools`).
- **`EXCEL_TOOLS_API_KEY`**: `dev-secret-key`
- **`MATH_TOOLS_URL`**: `http://<your-windows-local-ip>:8081` (Points to `fastapi-math-service`).
- **`MATH_TOOLS_API_KEY`**: `dev-secret-key`
