...
before the first ``from mas_agent_kit import ...`` in ``app.main``. Works the same for ``uvicorn app.main:app``,
pytest and the lab container (the kit is mounted at ``/mas-agent-kit``). Copy this file into your agent as is.
"""
...
load_service_env(Path(__file__).resolve().parents[1], "tnav-cluster.env")
