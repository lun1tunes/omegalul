...
- **Оркестратор удаляет `inputs.artifacts`** из `agent_task` — агенты берут карточки из `GET /cases/{id}/state`, а не из задачи.
...
- Один пункт плана = один новый чат. Первое сообщение — бриф по шаблону `/mas-brief` (цель, границы, файлы, критерий готовности, команды, запреты). Последнее сообщение агента должно содержать таблицу `mas_gate.py` и id live-кейсов — иначе работа не принята.
