...
- План (O13): `state.plan[{id,title,agent_id?,status,depends_on?,note?}]`, статусы `PLAN_STATUSES` (`mas_state_utils.py` — одна константа для схемы, JS и pytest-двойника `state_shape.sanitize_plan`). LLM пишет `plan_update` (id обязателен, без id → `rejected` в `orchestrator.decision.payload.plan`), `applyPlanUpdate` мержит **до** guard'ов; статусы ведут действия (`planMarkAgentActive` при `call_agent`, `planMarkAgentDone` в `applyAgentResult`); `finish` при `planOpenItems` → ветка `completion_unverified`/`completion_review`. План — блок «План задачи» в `planner_input` (попадает в `verify_input`), заголовки — в RAG-запрос, `feed.plan` → UI-блок «План».
...
- **Ответ инструмента с ключом `json.error` n8n 2.30.8 считает упавшим запуском ноды** и при `retryOnFail` шлёт тот же HTTP-вызов ещё раз — даже на HTTP 200. Так конверт `ToolError {"error": "result_already_stored"}` превращался в 3 «повторных `ask_engineer`» в логе (A14, `CASE-6a9fa62a-9a9a48`), хотя Qwen звала инструмент один раз. Конверт ошибки — `{ok:false, code, message, …}`; ключ `error` в ответе инструмента запрещён (`test_error_envelope_never_carries_the_n8n_failure_key`). Если в `trace.tool` подряд одинаковые вызовы с одинаковыми аргументами — сначала проверить, не ретрай ли это ноды.
...
- **Qwen пишет план прозой** (`plan_update: [{step, agent, action, reason}]` без `id`, `CASE-6a9fa129-5ae077`) — мерж по `id` молча терял всё. Любое поле от LLM, по которому мержат, объявлять в схеме `required` и считать отказы в payload решения, а не отбрасывать без следа.
