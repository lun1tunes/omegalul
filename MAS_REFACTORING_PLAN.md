# NOVATEK RE MASter — план развития «от рабочей альфы к умной MAS» (ревизия 58: ingest и UI знаний)

Ревизия 58 — 2026-09-19. 8.7: ingest по тексту/`content_hash`, `purge_superseded`, серверный поиск знаний, локальный `marked`/шрифты. История ревизий — §7; закрытый долг ревизий 1–42 — Приложение A; критерии закрытых фаз 0–6 — Приложение B (не переписываются). Как работать по плану — `AGENTS.md` (карта, инварианты, цикл задачи), гейт — `python3 scripts/mas_gate.py [--live [--repeat N]]` (`--bundle` — полевой zip, не стадия офлайн-гейта). Полевой гайд для инженера — `docs.md`. Очередь простыми словами — §2.8, бриф на пункт — `/mas-brief` → `briefs/`.

**Синхрон.** Шапка, §3 «Состояние», §4 сводка и критерий *текущего* гейта, §2.7, «Ближайшие шаги» (конец §7) = код на дату ревизии. Номер шапки = последняя запись §7. Журнал §7 и Приложения A/B не переписывать. Счётчики pytest в шапке не фиксировать — плывут; писать «smokes по glob, pytest-наборы гейта». `--live` сейчас **13 слотов**: шесть `run_live_five` + `demo_agent_long_job` + `three_agent_chain` + `excel_datasets` (extract + apply + дырка) + `tnav_cluster_run` + `agent_down_recovery` + `rework_round` + `step_limit_review`. `--repeat N` — шесть commissioning ×N в одном пуле, остальные сценарии один раз (не 13×N); стена ~25 мин. `--hot`, если lab здоров и JSON без дрейфа. **Требование заказчика: полный прогон (`--live`) ≤ 20 мин** — закрепляется как критерий гейта в Фазе 12 (T2), до этого — контроль по `LIVE_WALL_BUDGET_S`.

**Где проект.** Фазы 0–6.1 и альфа α1–α5 закрыты (Приложение B). Ревизия 58 — 8.7 ingest/UI знаний. Ревизия 57 — сверка docs/плана. Ревизия 56 — полевой env кластера и шаблона через kit (как Activity). Ревизия 55 — tNav Cluster Agent (10.1–10.2 и один live-слот 10.3) на текущем kit, раньше Фазы 9 по запросу заказчика. Ревизия 54 — оркестратор с политикой под цель (8.6). Ревизия 53 — корпус 8.5 + эмбеддинги `baai/bge-m3`. Ревизия 52 — KB по требованию (8.4). Ревизия 51 — RAG-фильтры из инвентаря (8.3). Ревизия 50 — live 12/12 на коде 8.1+8.2. Ревизия 49 — retrieval без ложного `SCHEMA_KEYWORD_SCOPE_REQUIRED`. Ревизия 48 — лог и честный отказ цикла. Ревизия 47 — hotfix цикла. Ревизия 46 — пороги LLM/RAG и матрица секретов. Ревизия 45 — HTTP-цикл агента. Ревизия 44 — лог RAG/LLM оркестратора. Ревизия 43 (аудит) остаётся диагнозом оркестраторского среза (три routing-карточки) и kit без типов. Очередь — §7 «Ближайшие шаги»: удаление `n8n_agent`, затем фазы 9–12.
...
Легенда статусов: ✅ сделано · 🟡 частично · ⬜ не начато · 🆕 найдено в ревизии 43 (аудит + live-executions), а не в предыдущих ревизиях.

---

## 0. Диагноз ревизии 43 — аудит всей системы

### 0.1 Что проверяли и чем

| Блок | Чем проверено | Артефакты доказательства |
|---|---|---|
| Оркестратор | чтение `generate_mas_orchestrator.py` (SYSTEM, `DECISION_SCHEMA`, `PARSE_DECISION`, `PREPARE_VERIFY`), `mas_state_utils.py`, `mas_retrieval_client.py`; executions `Orchestrator — MAS` #228559, #228501, #228497, #228414, #228363, #228320, #228288 (узел `Attach orchestrator RAG evidence`) | всегда те же 3 карточки из 8 найденных: `route-mas-thin-orchestrator`, `route-excel-extractor`, `route-schedule-builder`; `planner_input` 9.6–13.5 K символов |
| База знаний | `schedule_rag_workflows.py` (normalize, LEXICAL/TAG SQL, RRF, `ATTACH_SCHEMA_CATALOGUE`), корпус 59 карточек, executions `MAS — Knowledge Retrieval` (30 последних) | `schedule_mvp` без ALLCAPS-keyword в запросе → `status=abstain`, `SCHEMA_KEYWORD_SCOPE_REQUIRED`, 0 карточек (#228535, #228336); `excel_protocol` → всегда все 5 карточек; `orchestrator_routing` → 8 карточек до фильтра 45 % |
| Schedule Builder | ревизия сервиса и workflow; executions `Agent — Schedule Builder` #228529, #228330, #228230, #228121 (узел `Attach schedule RAG evidence`) | `rag.status=unavailable`, `cards=[]` во всех четырёх; `planner_input` ≈ 40 000 символов (`well_objects` до 200 скважин) |
| Excel-агент | ревизия сервиса, инструментов, карточек; executions `Agent — Excel Extractor` #228440, #228425, #228297 | 5/5 карточек `excel_protocol` каждый вызов, три из них описывают снятый протокол (`clr_`, `continuation_state`, `X-Excel-Webhook-Key`) |
| Activity backend + лог + UI | ревизия `cases_api.py`, `control_plane.py`, `case_log.py`, `case_watch.py`, `static/*`; лог `GET /cases/CASE-6aa6b6fc-3280a9/log` | в логе нет RAG, нет токенов на успешном решении, нет LLM-ходов агентов, нет длительностей между событиями |
| Kit / шаблон / спеки | ревизия `mas-agent-kit`, `agents-template/demo_agent`, `mas_agent_spec.py`, `mas_agent_workflow.py`, `test_service_shape.py`, `import-manifest.json`, `mas_gate.py` | pydantic в kit'е нет; для нового агента правятся ≥ 10 файлов вне его папки |
| LLM-транспорт | `llm_runtime_options.py`, JSON Chat Model агентов, model card Qwen3.6-27B | Chat Model агентов = `lmChatOpenAi` 1.3 без возможности выключить thinking; `CHAT_THINKING_OFF` без `chat_template_kwargs`; Qwen3.6 **не поддерживает** soft switch `/no_think` |

### 0.2 Вердикт по блокам

| Блок | Оценка | Главный факт |
|---|---|---|
| Оркестратор: решения, план, завершение | 🟢 архитектура верная, 🟡 «ум» не подкреплён знаниями | Домена в промпте нет, план и Verify работают, guards согласованы. Но RAG-срез статичен (3 карточки на любую цель), токены/`finish_reason` пишутся только при `unparsed`, `unlisted_wells_policy`/`isUnlistedWellsGate` — доменная утечка в `mas_state_utils.py` |
| База знаний | 🔴 не используется там, где обязана | Гибридный retrieval (FTS + PGVector + теги, RRF) есть, но: Builder получает **0 карточек** на всех commissioning-задачах; тег-ветка ищет только по фильтрам вызывающего, не по запросу; лексика — `websearch_to_tsquery` (AND) по 800-символьному запросу без русского стемминга; карточки режутся до 700–800 символов при медиане 2508; `worked_example` — 0; запрос и результат retrieval не логируются |
| Excel-агент | 🟢 инструменты и сессия, 🟡 гигиена | Детерминированный разбор книг хорош, `extract_table` закрыл произвольные наборы. Хвосты: 7 снятых инструментов ещё зарегистрированы, keyword-подсказки `suggested_select`, два стека дат (serial → `column_not_dates`), карточки RAG описывают снятый протокол и заморожены тестами |
| Schedule Builder | 🟢 движок SCHEDULE, 🔴 знания и общие операции | Commissioning/rebind/dataset детерминированны и покрыты. Но `apply_operations` игнорирует каталог схем (`_tokens_from_fields`), INCLUDE не раскрывается, planner получает 40 K инвентаря, RAG до модели не доходит, `search_keywords` — локальный лексический поиск без тегов |
| Activity backend | 🟢 контракт и прокси, 🟡 надёжность краёв | HITL-ответ и создание — fire-and-forget без записи намерения; «Продолжить» не появляется при болтливом агенте; кэш снимка не инвалидируется записями n8n; отмена не отгораживает старый execution |
| Лог разработчика | 🟡 форма есть, содержания не хватает | Шаги, решения, `trace.tool` — есть. Нет: RAG-запрос/карточки/статус, LLM-ходы агентов, токены и `finish_reason` на успехе, длительности между событиями, `orchestrator.resume` без ссылки на execution |
| Activity UI | 🟢 стиль и структура, 🟡 production-мелочи | Палитра и три панели верны. `log.js?v=1`, CDN-шрифты и `marked` (воздушный зазор на поле), пустые счётчики вкладок знаний, `CASE-…` и английские подписи в шапке, подсказка «ждём оркестратор» не гаснет при таймауте, страница «Агенты» перегружена JSON |
| Kit / шаблон / расширяемость | 🟡 рельсы есть, типов и полноты нет | Один класс `AgentService`, `ToolRegistry`, `SessionStore`, `ActivityClient` — хорошо. Нет pydantic-моделей контрактов (все dict), нет `LongJob`/heartbeat/cancel/recover, нет потокового I/O для бинарей, `test_service_shape` проверяет три дерева и не сверяет `TOOLS` спеки с реестром инструментов; манифест/Health/гейт/zip перечисляют агентов руками |
| LLM-транспорт (Qwen 3.6) | 🟡 оркестратор решён, 🔴 агенты — полевой блокер | Decision/Verify/Interpret — HTTP thinking off (α4). Chat Model агентов через `lmChatOpenAi` 1.3 отключить thinking не может; `CHAT_THINKING_OFF` не шлёт `chat_template_kwargs.enable_thinking=false` (единственный надёжный выключатель для vLLM/SGLang); агенты на `temperature=0` (model card для non-thinking: 0.7 / top_p 0.8 / presence 1.5, greedy → зацикливание) |

### 0.3 Факты, которые меняют очередь

1. **База знаний не доходит до Schedule Builder.** `ATTACH_SCHEMA_CATALOGUE` (`schedule_rag_workflows.py` ≈ 247): `if(!requested.length) findings.push({code:'SCHEMA_KEYWORD_SCOPE_REQUIRED'})` → `status:'abstain'`, `results:[]`. `keyword_families` заполняется только ALLCAPS-словами из текста задачи (`schedule_builder.py` 181–184); формулировка «сдвинуть даты ввода» их не содержит → 0 карточек. Live: retrieval #228535/#228336 `abstain`; Builder #228529/#228330/#228230/#228121 `rag.status=unavailable`. Attach fail-open — в логе следа нет.
2. **Оркестратор получает статичный срез.** RRF ранг-1 ≈ 1/61; порог 45 % от лучшего (`compactRetrievalCards`, `mas_retrieval_client.py` 105–143) оставляет только карточки, найденные всеми ветками, — на любой цели это три policy-карточки. Запрос из 800 символов (`buildRetrievalQuery`) в `websearch_to_tsquery('simple', …)` — AND всех токенов, практически не матчится; тег-ветка на шаге 0 пуста (`planRetrievalTopics` без плана → `[]`).
3. **Лог слепой на самом важном.** `orchestrator.decision.payload` несёт `llm_parse` только при `unparsed` (`generate_mas_orchestrator.py` 1158); нет вида события для retrieval и для LLM-хода агента; `EVENT_KINDS` Activity не знает `orchestrator.resume`, который n8n пишет в Postgres напрямую (802) без `execRef`. Разработчик видит «Решение → handoff → инструменты → результат», но не «что видел Qwen, что нашла база, почему».
4. **Thinking агентов — полевой блокер, и у него есть решение в рамках 2.30.8.** Decision уже ходит HTTP `/chat/completions`; тот же транспорт (HTTP Request + Code, цикл `tool_calls` ≤ `max_iterations`) заменяет `@n8n/n8n-nodes-langchain.agent` у Excel/Builder: три выключателя thinking (`reasoning.enabled=false`, `enable_thinking=false`, `chat_template_kwargs.enable_thinking=false`), профиль сэмплинга non-thinking, `trace.llm` на каждый ход, финальный текст модели в логе. Qwen3.6 soft switch `/no_think` не поддерживает (model card) — только параметры запроса.
5. **`planner_input` Builder'а ≈ 40 K символов** (#228529 — 39 997): `well_objects` до 200 скважин с полными событиями на каждом LLM-ходе. Это токены, латентность и шум, при том что `inspect_well` есть как инструмент.
6. **Regex вместо решений всё ещё есть** — не в маршрутизации, но в интерпретации: `isUnlistedWellsGate`/`readUnlistedWellsPolicy` в оркестраторе (`mas_state_utils.py` 318–322, 380–390); `suggested_select` по словам `скважин`/`дата`/`ввод` (`excel_tools.py` 932–1024); разбор HITL-подписи `остав|сохран|keep|remove` (`schedule-builder-service/app/agent_tools.py` 155–178); эвристики колонок в `io.py` 166–175, 203–218; RAG-фильтры по стемам (`excel_extractor.py` 168–172); `hasResult` по префиксу имени инструмента (`mas_agent_workflow.py` 307–314).
7. **Kit без типов.** `agent_task`, `agent_result`, `watch`, `requests[]`, аргументы инструментов — dict-фабрики; `js_format_result` — ручной JS-двойник allowlist (уже терял `watch`, `CASE-6a9f3bc9-10cb9f`); Activity `AgentResult`/`AgentRequest` — pydantic, но не совпадают с живым контрактом (`options: list[str]` vs `{value,label}`) и не применяются на HTTP-границе.
8. **Добавить агента = править ≥ 10 файлов вне его папки**: `agents/__init__.py` `ALL`, новый `generate_<x>.py`, `mas_gate.py` `GENERATORS` и `BUNDLE_EXTRAS`, `import-manifest.json` (`runtime_import_order`, `health_check.live_probes` — три агента руками), `docker-compose.yml`, `lab_soft_redeploy.py`, `test_service_shape.py` `SERVICES`, `test_workflow_contracts.py` (списки имён), новый smoke, `SELECTORS`/`NAMESPACES`/карточки, `docs.md`, live-харнесс. README шаблона обещает меньше.
9. **Долгий агент часами — kit не готов**: `threading.Thread` в памяти (умирает с процессом), нет heartbeat/cancel/recover, `finish_task` без retry и идемпотентности (второй POST = второй resume и наложение `data`), бинарные артефакты — целиком в памяти и BYTEA через прокси. Кластерный агент на этом не построить.
10. **Гейт укладывается в 20 минут только одиночным `--live`** (шесть commissioning 63–115 с параллельно, остальное последовательно); `--repeat 3` = 1219 с. Требование «≤ 20 мин» — закрепить как критерий (`LIVE_WALL_BUDGET_S`), `--repeat 3` — отдельный ночной режим.

---

## 1. Что сделано правильно (не трогать)

- **Тонкий оркестратор без домена**: SYSTEM без имён агентов и маршрутов, возможности — из `agent_registry`, политика — из RAG; план как объект (`plan_update` по `id`), журнал, `Verify completion` детерминированно из частей; guards читают план (O22). Ошибки решений именованы (`decision_unparsed`, `llm_truncated_empty`, `provider_busy`).
- **Decision/Verify/Interpret на HTTP `/chat/completions` с thinking off** — единственный работающий на поле путь для Structured Output с Qwen 3.6 (α4). Расширять на агентов, не откатывать.
- **Control Plane Proxy** как единственная дорога в Postgres; статусы кейса и CHECK — из одной константы.
- **Детерминированный движок SCHEDULE**: parse/apply/emit, блочный `/`, порядок внутри DATES, commissioning, group rebind, `apply_dataset` по каталогу схем (100 вариантов), семантическое сравнение `.INC`.
- **Гибридный retrieval с изоляцией**: FTS + PGVector + теги, RRF k=60, `target_base`/`knowledge_types`/`access_scope`/`knowledge_status=current`, гидратация полной карточки, ревизии и `superseded`. Чинить надо ранжирование и «когда воздерживаться», не архитектуру.
...
- **Гейт**: regen drift, smokes по glob (сейчас 18), pytest-наборы гейта (kit, Activity, Schedule, Excel, tNav Cluster, demo), офлайн combat, **13 live-слотов** с порогами α2 (`rag_empty: 0`) и семантическим сравнением; отказные сценарии (`agent_down_recovery`, `rework_round`, `step_limit_review`).
...
## 2. Инвентарь долга ревизии 43 (активный)

Формат строки: код · где (файл:строки) · факт · как закрыть · «готово, когда». Коды продолжают нумерацию Приложения A (O24+, K8+, T2+), новые серии: R (retrieval/знания), X (Excel), S (Schedule Builder), B (Activity backend), U (UI), Q (LLM-транспорт). Все строки 🆕 ревизии 43, если не указано иное.

### 2.1 Оркестратор и решения LLM

| # | Где | Факт | Как закрыть | Готово, когда |
|---|---|---|---|---|
| O24 | `mas_retrieval_client.py` `SELECTORS.orchestrator`, `compactRetrievalCards`; `mas_state_utils.py` `buildRetrievalQuery`, `planRetrievalTopics` | ✅ ревизия 54: запрос = цель + заголовки плана + открытый HITL; `topics` = роли файлов ∪ открытого плана ∪ `completion`; orch+caller topics → keep только overlap (иначе 2-ветка 45 % оставляла excel на rebind, `CASE-6aacea63-31c63b`); файловые теги не сбрасываются, когда пункт агента `done` (`CASE-6aacef32-374aa4`) | — | golden_1 `CASE-6aacf221-68489c` excel+thin+schedule; golden_2 `CASE-6aacf221-9eafe8` thin+schedule; demo `CASE-6aacf2bb-37b929` `orch_rag_ids: []` |
...
| O28 | `PREPARE_VERIFY` | ✅ ревизия 54: Verify получает срез `topics: completion` («Политика завершения»), не факты агентов | — | smoke `verify_input`; commissioning `mismatch_count: 0` |
| O29 | `BUILD_DECISION_CHAT` / `planner_input` | ✅ ревизия 54: Цель → План → Журнал → Данные агентов → Открытые вопросы → Реестр → Политика из базы знаний | — | smoke порядка секций; commissioning `steps ≤ 6` |
| O30 | SYSTEM | ✅ ревизия 54: политика из базы знаний важнее догадки; HITL если политика требует уточнения; план по пунктам политики | — | chain `CASE-6aacf2bb-eb77f8` план 3/3; commissioning план excel→builder при excel-карточке; rebind без excel_extractor без xlsx |

### 2.2 База знаний и retrieval
...
| R11 | `SELECT_NEW_KNOWLEDGE_JS`, `LOOKUP_EXISTING_SQL`, inventory | Skip по тройке `(base, id, revision)` игнорирует текст/`content_hash`; `superseded` чанки не удаляются; skipped id считались missing (ложный 61/61 incomplete) | ✅ ревизия 58: skip только `in_vector` и тот же текст/hash (иначе `CONTENT_HASH_CHANGED_BUMP_REVISION`); `purge_superseded`; skipped id = found | Smoke: другой текст при том же id/rev → `needs_input`; inventory skip-as-found ok |
| R12 | `knowledge.py` `list_documents`/`list_namespaces`, `knowledge.js` | Поиск — подстрока по 160-символьному превью на клиенте; `document_count` не отдаётся; нет истории ревизий, per-card ingest, смены типа/статуса, просмотра `schema_catalogue` | ✅ ревизия 58: `GET /v1/knowledge/{base}?q=&tag=`, `document_count`, `GET …/revisions` через прокси, ingest одной карточки, schema/examples read-only | `test_knowledge_api`; live probe `?q=DATES`; UI: теги, ревизии, «Загрузить эту карточку» |
...
| # | Где | Факт | Как закрыть | Готово, когда |
|---|---|---|---|---|
| X1 | `excel-agent-tools/app/state_tools.py`, `excel_tools.py` 1743–1867, `agent.py` 176–192 (`save_agent_plan`), `/health.tools` | 7 снятых инструментов (`get_session_state`, `save_agent_plan`, `submit_clarification`, `resolve_clarification`, `finalize_extraction`, `validate_result`, `export_result`) зарегистрированы; `/health.tools` ≠ n8n `TOOLS` | Снять регистрацию (или за `EXCEL_LEGACY_API=1`); убрать алиасы из `normalize_args`; тест `health.tools == spec.TOOLS` | `/health.tools` равен `TOOLS` спеки (pytest + `test_service_shape` K10) |
| X2 | `excel_tools.py` 932–1024 (`_is_well_identity_column`, `_is_commissioning_date_column`, `_looks_schedule_fact_query`) → `suggested_select` | Подсказка колонок по словам `скважин`/`дата`/`ввод` — выбор за LLM делает regex; карточка RAG велит доверять | `match_tables` возвращает ранжирование таблиц как **справку** (`advisory: true`), без предвыбора колонок; выбор колонок — LLM по инвентарю; `test_messy_layouts.py` 243 переписать | `match_tables("даты ввода")` не добавляет колонки, которых нет в запросе; live 6/6 без роста `tool_calls` |
| X3 | `agent_tools.py` `_as_date` vs `dataset_extract.as_date` (Excel serial принимает только второй) | Два стека дат; serial-даты после `data_only` → `column_not_dates` | Один кастер (`dataset_extract.as_date`); serial (20000–80000) допустим, когда LLM назвала колонку датой | Pytest: serial 44927 + колонка дат → `YYYY-MM-DD`; колонка дебитов — `column_not_dates` |
...
| S5 | `agent_tools.py` 155–178 (`остав|сохран|keep|remove`), `io.py` 166–175, 203–218 | Разбор подписи HITL и эвристики колонок Excel-превью | Только `choice`/`inputs.unlisted_wells_policy` (после O26 — только `choice` из `context.hitl.answers`); колонки — только из `facts`/наборов, без эвристик | Pytest `test_unlisted_policy_from_plain_hitl_string` переписан на `choice`; grep стемов пуст |
| S6 | Builder tools не зовут `repeat_guard`; `maxIterations 8` vs α2 `tool_calls ≤ 4` | Петля `inspect_*` может съесть бюджет | `repeat_guard` на `inspect_*`/`list_records` (3), `too_many_attempts` с подсказкой | 4-й `inspect_well` → `too_many_attempts`; live `tool_calls ≤ 4` |
| S7 | `apply_operations` 876–908 (последний блок keyword) | MODIFY без даты правит последний блок глобально | Обязательное поле `date` у операции (или `effective_at`) → сегмент DATES; без даты → `spec_incomplete` | Pytest: MODIFY WCONPROD на `1 FEB 2026` не трогает январь |
| S8 | `list_records` token[0] как `well` | Неверно для GRUPTREE/DATES/GCONPROD | Ключ записи — по схеме keyword (`identity` из каталога) | Pytest на GRUPTREE |
| S9 | `store_result` — `schedule_out`/`diff` inline в `agent_result` | Мегабайты `.INC` едут через n8n item и state | `ActivityClient.upload` → карточка `schedule_out`; в `agent_result.artifacts` — ссылка | Live: `artifacts.schedule.out` — карточка, deliverable скачивается; state кейса без полного текста |
| S10 | `KEYWORDS` (R14), `WITHIN_DATE_KEYWORD_ORDER` vs `schedule_emit_order.py`, `FIELDS` vs каталог vs `_tokens_from_fields`, `TOOLS` в трёх местах | Двойники и тройники | R14 + S2 + K9 (генерация `@tool`-схем из спеки или наоборот — один источник) | Тесты равенства; regen drift 0 |
| S11 | `agent.py` `open_session` не использует kit `packet()`/`base_state`; `io.bind_case_packet` копирует в `context.data.excel` | Builder мимо общей формы kit'а | Перейти на `self.packet(task)` + `base_state`; доменные поля поверх | `test_service_shape` расширен (K10) зелёный; pytest Builder зелёный |
| S12 (объём, не долг) | — | Не поддержано: compose WCONINJE, перенос/удаление в GRUPTREE, вставка DATES для ops, `.dev`/WELLTRACK точки, тело VFP, ACTIONX/UDQ программы, TSTEP, COMPDAT (TVD), WELOPENL | Брать по одному по запросу инженера: allowlist → карточка → схема → детерминированный apply → live-кейс | Каждый — отдельная строка при взятии в работу |

### 2.4 Activity backend и лог разработчика

| # | Где | Факт | Как закрыть | Готово, когда |
|---|---|---|---|---|
| B1 | `cases_api.py` `post_answer` ≈ 1005–1028 (`background_tasks.add_task(_invoke_resume)`), `create_case` | HITL-ответ и создание — fire-and-forget: 200 при статусе `waiting_user`, `hitl.answered` пишет только оркестратор; падение Activity/недоступный webhook → ответ потерян; двойной клик → два resume | Синхронно писать событие `hitl.answered` (или `orchestrator.status queued`) + `version++` **до** вызова; single-flight на кейс (lock по `case_id`); второй ответ до потребления → 409 | Тест: убить Activity после 200 → ответ есть в событиях; повторный POST → 409 |
...
| B12 | `schema_view.py` `AGENT_NODES` vs `schema.js` (реестр) | 🟡 ревизия 55: узел `cluster` / `tnav_cluster` в Python. UI рисует по реестру; Python-карта всё ещё руками (нет `demo_agent` как отдельного узла) | Удалить `feed.schema` из Python или генерировать из реестра | новый агент на схеме без правки `schema_view.py` |

### 2.5 Activity UI

| # | Где | Факт | Как закрыть | Готово, когда |
|---|---|---|---|---|
| U1 | `index.html` 287 `log.js?v=1` | Cache-bust не бампается → устаревший рендер лога у инженера | Хэш содержимого в `?v=` при сборке страницы (Python отдаёт `index.html` с подстановкой) или тест «изменился файл — изменился v» | `test_activity_api`: `?v=` каждого static = его хэшу |
| U2 | `index.html`/`knowledge.html` (Google Fonts, jsDelivr `marked@15.0.7`) | Поле без интернета: шрифты — fallback, markdown карточек — `<pre>` | ✅ ревизия 58: `static/vendor/marked.min.js` + IBM Plex woff2; HTML без `fonts.googleapis`/`jsdelivr` | `test_knowledge_page_assets`; live probe HTML |
| U3 | `knowledge.py` (`document_count` нет), `NAMESPACE_LABELS` (EN/RU) | Пустые бейджи вкладок, смесь языков | ✅ ревизия 58: `document_count`; подписи «Сборщик расписания», «Извлечение из Excel», «Маршрутизация», «Шаблон специалиста» | live probe `schedule_mvp` label + count |
...
| U9 | = R12 | Знания: поиск/теги/ревизии/ingest карточки | ✅ ревизия 58: см. R12 | см. R12 |
...
### 2.6 Kit, спеки, расширяемость

| # | Где | Факт | Как закрыть | Готово, когда |
|---|---|---|---|---|
| K8 | `mas-agent-kit/mas_agent_kit/{result,hitl,dataset,agent}.py` (dict-фабрики), `AgentSpec` (`@dataclass`) | Ни одной pydantic-модели контракта; `watch`/`requests`/`data` не валидируются | `models.py` (pydantic v2): `AgentTask`, `AgentResult`, `EngineerRequest{options[{value,label}]}`, `Watch{kind,ref,poll_hint,eta?}`, `ToolErrorBody`, `OpenedSession`, `DatasetEntry`, `JobSpec` (K12); фабрики возвращают `model_dump(exclude_none=True)` с **тем же JSON**; `@tools.tool(args_model=…)` после `coerce_args`; Activity импортирует kit (B11) | Kit-тесты + HTTP-фикстуры сервисов зелёные без изменений JSON; `test_agent_result_contract` — через модель |
| K9 | `mas_agent_workflow.py` `js_format_result` 381–394 (ручной allowlist), Activity `AgentResult`, `applyAgentResult` | Три писателя одного контракта; новое поле — три правки | Генерировать allowlist JS и константу полей из `AgentResult.model_fields`; smoke сверяет | Добавление поля в модель без регенерации → красный regen drift |
| K10 | `test_service_shape.py` (3 дерева; не сверяет `TOOLS`↔реестр, `/health`, генератор, smoke) | «Одна форма» проверяется частично | Параметризовать по `agents.ALL` (`has_workflow`): `spec.TOOLS ⊆ agent.tools.names`; `ask_engineer` при `agent_asks`; `/health` с `mas_version`; есть `generate_<id>.py`, smoke, `service_url_key` в Runtime Config, `.env.example`, `start-windows.bat`; `enabled` ⇒ проба Health Check | Спека без генератора/smoke — красный гейт |
| K11 | `agents/__init__.py` `ALL`; `mas_gate.py` `GENERATORS` 51–61, `BUNDLE_EXTRAS` 382–405; `import-manifest.json` (`runtime_import_order`, `health_check.live_probes` 94–101); `generate_mas_health_check.py` MANUAL 362–368; `docker-compose.yml`; `lab_soft_redeploy.py`; `test_workflow_contracts.py` 403–415, 490–514 | ≥ 10 файлов вне папки агента | Discovery `agents/*.py`; манифест, пробы Health, MANUAL-чеклист, `GENERATORS`, `BUNDLE_EXTRAS` — производные от спек (`service_dir`, `enabled`); compose-сниппет печатается, не патчится; списки имён в тестах — из `agents.ALL` | Новый агент = папка сервиса + `agents/<id>.py` + `generate_<id>.py`; `git diff --stat` вне них ≤ 3 файла (compose, docs, live-харнесс) |
| K12 | `demo_agent/app/agent.py` 62, 130–157 (`threading.Thread`, dict `jobs`); `ActivityClient.finish_task` 213–222 | Долгое задание умирает с процессом; нет heartbeat/cancel/recover; `finish_task` без retry и идемпотентности | `jobs.py`: `JobSpec` (pydantic), `JobStore` на диске (рядом с сессией), `AgentService.start_job/recover_jobs/heartbeat/should_cancel` (опрос `GET /cases/{id}/state` на heartbeat), `finish_task` с retry/backoff и `result_id`; оркестратор игнорирует повторный `result_id` для `task_id` | Тест: `kill` процесса demo во время job → рестарт → `finish_task` один раз; cancel кейса останавливает цикл |
| K13 | `ActivityClient.download/upload` (bytes целиком, 60/120 с); `role_for_artifact_id` в kit/`state_shape.py`/`mas_state_utils.py` | Бинарные модели (`.DATA`, restart, UNSMRY) не пройдут через память/BYTEA; роли — три копии | `upload_file(path)`/`download_to(path)` потоково; `inputs` допускает путь на общем диске (поле); роли артефактов — одна таблица → три двойника генерируются/тестируются | 100 МБ round-trip в pytest; новая роль — одна правка |
| K14 | Excel `main.py` (свой `/health`), Schedule свой, Math `{status,service}`, kit `create_agent_app` | 🟡 ревизия 56: demo и кластер — `create_agent_app` (`ok`, `agent_id`, `tools`, `mas_version`, `env_files`). Excel/Schedule/Math — свои формы | Все агенты через `create_agent_app(..., extra_health=…)` | `test_service_shape`: `/health` всех агентов содержит `ok`, `agent_id`, `tools`, `mas_version` |
| K15 | `EVENT_KINDS` kit ⊂ Activity (две таблицы); `boundedForLog` vs `compact_for_log`; `looksMachineText` vs `human_text_problems` | Двойники с расхождениями | Одна таблица видов в kit, Activity импортирует и объединяет с оркестраторными; тест `kit ⊆ activity`; JS-двойники получают тесты равенства на фикстурах | Новый вид в kit без Activity → красный pytest |
| K16 | `fastapi-math-service/app/main.py` (pydantic `AgentTaskBody`, без kit, без `agent_id` в ответе) | Четвёртая форма сервиса | `CalculationAgent(AgentService)` с одним инструментом **или** явное исключение `invoke.kind=http` в `test_service_shape` | Math в `test_service_shape` или исключён явно |
...
### 2.7 Сводка пробелов: что мешает «умной» системе и масштабированию (ревизия 58)

LLM-транспорт Qwen (Q) и гейт/документация (T) — здесь же, как сквозные.

| # | Где | Что | Статус / как закрыть |
|---|---|---|---|
| 🆕 Q1 | Excel/Schedule/Demo JSON | ✅ ревизия 45: `http_loop`; 🟡 двойной граф `n8n_agent` ещё в генераторе | ⬜ ревизия 59: удалить `llm_transport=n8n_agent` |
...
| 🆕 T4 | `.cursor/rules/excel-agent-tools.mdc`, `AGENTS.md` §6 «Новый агент» | 🟡 ревизия 57: правило Excel — конверт `{ok:false, code, …}` без ключа `error`. Рецепт нового агента всё ещё не перечисляет правки вне папки (K11) | ⬜ Фаза 9 по факту закрытия K11: рецепт = три файла + список «что ещё» |
| 🆕 T6 | `excel-agent-tools/start-windows.bat`, `schedule-builder-service/start-windows.bat` | CMD `for /f` — тот же полевой дефект, что у кластера до ревизии 56 (BOM, пароль с `=`) | Как Activity/кластер/шаблон: `python -m app` + kit `load_service_env` | bat без `for /f`; pytest shape на Excel/Schedule |
| 🆕 T5 | live-харнессы (`run_live_five.py`, `run_live_excel_datasets.py`, `run_live_tnav_cluster.py`), `case_log.threshold_violations` | ✅ ревизия 52: `max_rag_empty=0` на commissioning/datasets; `excel_apply_dataset` `min_kb_calls=1`. ✅ ревизия 53: commissioning шаг 0 `trace.rag` ⊆ четырёх policy-карточек. ✅ ревизия 54: golden_1 ≠ golden_2 `orch_rag_ids`; file-less demo не несёт сразу excel и schedule. ✅ ревизия 55: слот `tnav_cluster_run` (13-й). ✅ ревизия 58: `--live` сначала knowledge probe (без Google Fonts/jsDelivr, RU `document_count`, `?q=DATES`) | — |
...
| F4/F5/F6 (из Приложения A) | метрики, Health Check по живому реестру, боевой агент | 🟡 F6: tNav Cluster на kit v1 (ревизия 55); live-слот стабилизирован в ревизии 58 (function calling в промпте, сброс leftover-файлов мока). F4/F5 ⬜ | Фазы 12, 9; хвост 10.3 |

### 2.8 Очередь и хвосты — простыми словами

**Что закрыто (фазы и альфа).** Стабильный контур «Excel → вопросы инженеру → новый `.INC`» на поле; оркестратор с планом и проверкой; долгие агенты; реестр; deliverables; лог; таблица → схема → `.INC`; live-гейт **13/13** (ревизия 55; альфа 12/12 — Приложение B, журнал §7 ревизии 2–42). **7.1+7.2 (ревизия 44):** `trace.rag`/`trace.llm`, payload решения с RAG/токенами, `orchestrator.resume` с execRef, рендер в логе. **7.3+7.4 (ревизия 45):** HTTP-цикл агента, thinking off, `SAMPLING`, `retrieve_knowledge` в графе, X7 из сессии, `trace.llm` `role=agent`. **7.5+7.6 (ревизия 46):** порог `llm_truncated: 0`, счётчики `rag_empty`/`kb_calls` в `report.json`, матрица секретов, полные `.env.example`. **7.7 (ревизия 47):** hotfix цикла (валидный JSON инструмента, человеческий `agent.progress`, `failed`≠`abstain`, `kb_calls` on-demand, status в `/log`). **7.8 (ревизия 48):** порядок RAG/LLM до decision, `prompt_preview` блоками, `llm_unavailable` / mid-loop `service_unreachable`, `chat_extra_params`, матрица function calling. **8.1+8.2 (ревизия 49):** retrieval без ложного schema-abstain, теги/русский FTS/`summary`, `rag_empty: 0`. **8.3 (ревизия 51):** фильтры RAG из инвентаря. **8.4 (ревизия 52):** `retrieve_knowledge` в цикле, `kb_calls ≥ 1` на apply. **8.5 (ревизия 53):** корпус R6–R8; эмбеддинги `baai/bge-m3` / `tnavigator_schedule_knowledge_v2`. **8.6 (ревизия 54):** политика оркестратора под цель (разные `orch_rag_ids`). **tNav Cluster (ревизия 55, раньше kit v2 по запросу заказчика):** сервис `tnav-cluster-service`, workflow, слот `tnav_cluster_run`. **Полевой `*.env` (ревизия 56):** кластер и шаблон `demo_agent` читают env через kit `load_service_env` (`utf-8-sig`), `start-windows.bat` = `python -m app`; Excel/Schedule ещё парсят CMD `for /f`.
...
3. **Потом пороги и матрица секретов — ✅ ревизия 46.** `llm_truncated: 0` в гейте; `rag_empty`/`kb_calls` считаются. **Hotfix цикла — ✅ ревизия 47.** **Лог и честный отказ — ✅ ревизия 48.** **Оживить базу — ✅ ревизия 49 (8.1+8.2).** **Live 12/12 — ✅ ревизия 50.** **Фильтры RAG из инвентаря — ✅ ревизия 51 (8.3).** **KB по требованию — ✅ ревизия 52 (8.4).** **Корпус + bge-m3 — ✅ ревизия 53 (8.5).** **Оркестратор с политикой под цель — ✅ ревизия 54 (8.6).** **Ingest и UI знаний — ✅ ревизия 58 (8.7).** Фаза 8 закрыта.
4. **Затем kit v2.** Фаза 9: pydantic-контракты (K8–K9), «чеклист нового агента» как тест (K10), discovery спек и производные манифест/гейт/zip (K11), `LongJob` и потоковые бинари (K12–K13), одна форма `/health` и событий (K14–K15). Кластерный агент уже на kit v1 — перенос на `LongJob` остаётся в 10.3/9.5.
5. **Хвост кластерного агента** (Фаза 10.3): четырёхагентная цепь, отмена во время расчёта, пространство `cluster_ops` вместо `specialist`.
6. **Activity до production** (Фаза 11): надёжность краёв (B1–B4, B7), UI-мелочи (U1–U8), страница «Агенты» для инженера.
7. **Гейт ≤ 20 мин, метрики, golden-2** (Фаза 12): пороги знаний/LLM, страница «Метрики», новые live-кейсы, таймауты вниз.

**Не очередь, но не забывать.** Безопасность Activity (B8) — вне объёма по решению заказчика. Судью «красиво ли написано» не добавлять. S12 — объём домена Builder'а, брать по запросу.

---

## 3. Целевая архитектура (принципы 1–11 сохранены, 12–16 добавлены)

1. **Оркестратор доменно-нейтрален**: ни агентов, ни маршрутов, ни keyword'ов в промпте; возможности — из реестра, политика — из базы знаний.
2. **Реестр исполняем**: `invoke`, `input_required`, `output_provides`, схемы, `hitl_policy`, `enabled`; привязка id — из UI.
3. **Агент = LLM + инструменты FastAPI**; выбор инструмента, таблицы, колонки, интерпретация ответа инженера — LLM; regex только как гейт.
4. **HITL — диалог инженера**: русская проза + `options[{value,label}]`; инженер даёт факты, никогда не пишет `.INC`/JSON.
5. **Один контракт агента** `agent_task → agent_result`; агенты не зовут друг друга; долгий агент — `in_progress` → `waiting_agent` → `finish_task`.
6. **Лента событий — источник истины** для UI, лога и метрик; `trace.*` — только разработчику.
7. **Память и проверка**: план как объект, журнал, `Verify completion` из частей; лимит шагов — предохранитель.
8. **Live-гейт — определение «готово»**; семантическое сравнение `.INC`.
9. **Deliverables — артефакты агентов** (`POST /cases/{id}/artifacts`), не поля state.
10. **Долгие агенты — граждане первого класса**: `watch`, heartbeat, cancel, recover.
11. **Техническое — в лог, человеческое — в чат.**
12. 🆕 **База знаний — активный участник, не приложение к промпту.** Retrieval даёт карточки на каждой задаче Builder'а и разные политики на разные цели оркестратора; агент может спросить базу в середине работы (`retrieve_knowledge`); каждый запрос и каждый ответ базы — событие лога (`trace.rag`). Пустой срез там, где карточки есть, — дефект гейта (`rag_empty: 0`), не «fail-open».
13. 🆕 **Контракт наблюдаемости.** Для каждого LLM-хода — `trace.llm` (модель, токены, `finish_reason`, длительность, инструменты, bounded-промпт); для каждого решения оркестратора — токены и RAG в payload; для каждого события — длительность от предыдущего; для каждого события n8n — ссылка на execution. Разработчик отвечает на «что видела модель, что нашла база, сколько это стоило» из Activity, не из n8n.
14. 🆕 **Один транспорт LLM — HTTP `/chat/completions` с thinking off** — для Decision/Verify/Interpret **и** для цикла агентов; профиль сэмплинга per роль; `lmChatOpenAi`/LangChain-агент n8n не используются, пока 2.30.8 не умеет `chat_template_kwargs`.
...
**Состояние (ревизия 58).** Принципы 1–11 подтверждены; live-гейт 13/13 GREEN (ревизия 55 слот кластера; ревизия 58 — 8.7 + повтор 13/13). Полевой env кластера и шаблона — Python dotenv как Activity (ревизия 56). Docs/план/правила сверены с кодом (ревизия 57). Принцип 12: Builder на commissioning получает карточки (`trace.rag` `ready`, `rag_empty: 0`); фильтры среза — из инвентаря (8.3); `retrieve_knowledge` в цикле — live `kb_calls ≥ 1` на apply (8.4); корпус без снятых контрактов, эмбеддинги `baai/bge-m3` на `tnavigator_schedule_knowledge_v2` (8.5); политика оркестратора под цель — разные `knowledge_id` на commissioning / rebind / file-less demo (8.6); ingest по тексту/`content_hash`, `purge_superseded`, UI знаний с серверным поиском и локальным `marked` (8.7). Принцип 13: лог RAG/LLM перед решением. Принцип 14: HTTP-чат, thinking off. Принципы 15–16 — фазы 9–12. Кластерный агент на kit v1 (не LongJob). Фаза 8 закрыта.

---

## 4. План по фазам (ревизия 58)

**Сводка.** Фазы 0–6.1 и α1–α5 закрыты — критерии в Приложении B. Фаза 7 закрыта (ревизии 44–48). Фаза 8 закрыта: 8.1+8.2 (ревизия 49), live 12/12 (ревизия 50), 8.3 (51), 8.4 (52), 8.5 (53), 8.6 (54), 8.7 (58). tNav Cluster (10.1/10.2 и один слот 10.3) — ревизия 55 на kit v1, live 13/13. Полевой env кластера и шаблона — ревизия 56. Сверка docs/плана — ревизия 57. Остаток B6 (`llm_ms`/`tool_ms`) и U7 — фаза 11. **Текущий гейт:** `mas_gate.py` (smokes по glob, pytest-наборы включая tNav Cluster, combat) + `--live` 13/13, knowledge probe, `mismatch_count: 0`, α2 (`warnings: 0`, `steps ≤ 6`, `tool_calls ≤ 4`), `llm_truncated: 0`, `rag_empty: 0`; commissioning-лог: `trace.rag`/`trace.llm` **перед** `orchestrator.decision`, Builder `trace.rag` `ready`; шаг 0 ⊆ четырёх policy-карточек; golden_1 ≠ golden_2 `orch_rag_ids`; file-less demo не несёт сразу excel и schedule; `prompt_preview.messages`; отказ чата не маскируется под «не вернул факты»; в JSON агентов нет `lmChatOpenAi`; `excel_apply_dataset` `kb_calls ≥ 1`; `tnav_cluster_run` — `waiting_agent` → `run_status=finished`. Оценка объёма — в «сессиях» (одна сессия = один бриф до зелёного гейта).
...
| 8 Живая база знаний | Builder и оркестратор получают нужные карточки; KB по требованию; чистый корпус; знания в UI | R1–R8, R10–R14, O24, O28–O30, S1, X-карточки | ✅ 49–54, 58 |
| 9 Kit v2 | pydantic, чеклист-тест, discovery, LongJob, бинари, одна форма | K8–K18, X1, X3, X6–X9, S5–S6, S9–S11, B11–B12, T4 | 7–9 |
| 10 Кластерный агент | первый боевой агент по спеке заказчика на текущем kit | F6, R8 (замена заглушки), новый live-слот | 🟡 55 |
...
**Границы:** RAG-ранжирование не трогать (Фаза 8); спеки и FastAPI агентов не менять, кроме X7 (результат из сессии).

### Фаза 8 — Живая база знаний ✅

**Зачем.** Принцип 12: Builder получает знания, оркестратор — политику под цель, модель может спросить базу.
...
6. **8.6 Оркестратор и знания** — ✅ ревизия 54. O24, O28, O29, O30: запрос без файлов/журнала; `topics` из файлов + открытого плана + `completion`; overlap-only compact на `orchestrator_routing`; секции `planner_input`; SYSTEM/Verify про политику.
   *Готово:* golden_1 ≠ golden_2 `orch_rag_ids`; demo без пары excel+schedule; chain план 3/3 `rag_empty: 0`; `steps ≤ 6`; `warnings: 0`.
7. **8.7 Ingest и UI знаний** — ✅ ревизия 58. R11, R12/U9, U3: skip ingest только при том же тексте/`content_hash`; иначе `CONTENT_HASH_CHANGED_BUMP_REVISION`; `purge_superseded`; серверный поиск `?q=&tag=`; `document_count`; ревизии через прокси; ingest одной карточки; локальный `marked` и IBM Plex (U2).
   *Готово:* `test_knowledge_api`, `schedule-rag-runtime-smoke.js`, knowledge probe в `--live`; Activity `/knowledge` без googleapis/jsdelivr. Live 13/13 `CASE-6aae475d-*` / `CASE-6aae47f3-*` / `CASE-6aae480f-4f6c99` / `CASE-6aae488f-4357e2`.

**Критерий фазы:** `--live` 13/13 с порогами `rag_empty: 0`, `kb_calls ≥ 1` на `excel_apply_dataset`; Builder на всех commissioning видит карточки; оркестратор получает разные политики; корпус без снятых контрактов; knowledge UI с поиском по тегам; ingest skip по тексту/`content_hash`.

**Границы:** новые keyword'ы SCHEDULE (S12) не добавлять; PDF мануала не ингестить (отдельное решение заказчика — объём и лицензия).

### Фаза 9 — Kit v2: типы, чеклист, discovery, долгие задания ⬜

**Зачем.** Принципы 15–16: заказчик добавляет агента сам; кластерный агент не упирается в kit.

**Deliverables:**

1. **9.1 Pydantic-модели контрактов** — K8: `mas-agent-kit/mas_agent_kit/models.py`; фабрики через модели с тем же JSON; `args_model` в `ToolRegistry`; Activity импортирует kit (B11) и валидирует `POST /run source=agent`, `POST /events`; мёртвые статусы/модели Activity удалены.
   *Готово:* JSON-фикстуры всех сервисов без изменений; `AgentResult` один; `AgentRequest.options` = `{value,label}`.
2. **9.2 Двойники из модели** — K9, K15, K13 (роли): `js_format_result` allowlist, константа полей для `applyAgentResult`, таблица `EVENT_KINDS`, таблица ролей артефактов — генерируются в JS из Python, smoke/pytest сверяют.
   *Готово:* правка модели без регенерации → regen drift красный.
3. **9.3 Чеклист нового агента как тест** — K10, K14, K16, K17: `test_service_shape` по `agents.ALL`; `/health` через `create_agent_app(extra_health)`; Math — в тест или явное исключение; demo на `specialist`.
   *Готово:* спека без генератора/smoke/`.env.example`/bat → красный гейт.
4. **9.4 Discovery и производные** — K11, K18: `agents/*.py` обнаруживаются; `generate_import_manifest.py`; Health MANUAL и пробы, `GENERATORS`, `BUNDLE_EXTRAS`, списки имён в тестах — из спек; compose-сниппет печатается.
   *Готово:* тестовый агент во временной папке: `git diff --stat` вне его файлов ≤ 3 (compose, docs, live-харнесс).
5. **9.5 `LongJob`** — K12: `jobs.py`, `JobStore`, `start_job/recover_jobs/heartbeat/should_cancel`, `finish_task` retry + `result_id`, оркестратор игнорирует повтор; demo переписан на `LongJob`.
   *Готово:* pytest с рестартом процесса и cancel; `demo_agent_long_job` live GREEN.
6. **9.6 Бинари** — K13: `upload_file`/`download_to` потоково; путь на общем диске в `inputs`; лимиты документированы.
   *Готово:* 100 МБ round-trip в pytest.
7. **9.7 Агенты на общей форме** — S11 (Builder через `packet()`/`base_state`), X1 (снять инструменты), X3/X6 (даты, коды), X8 (`args_model`), S5/S6 (regex HITL и `repeat_guard`), S9 (upload `schedule_out`), B12 (`schema_view`).
   *Готово:* pytest Excel/Builder/Activity; `--live` 12/12.
8. **9.8 Документация** — T4: `AGENTS.md` §6 рецепт = «спека + папка + генератор»; `.cursor/rules/excel-agent-tools.mdc` конверт; README шаблона.

**Критерий фазы:** «новый агент» из README за ≤ 1 час без правок оркестратора/Activity/UI и с ≤ 3 внешними правками; kit-модели — единственный источник контракта; долгий агент переживает рестарт.

**Границы:** доменная логика агентов не меняется; форматы `.INC` не трогаются.

### Фаза 10 — Кластерный агент (первый боевой) 🟡

**Зачем.** Спека заказчика: работа с файлами модели на кластере, запуск tNavigator CLI, наблюдение, передача результата.

**Deliverables:**

1. **10.1 Спека и сервис** — ✅ ревизия 55 на kit v1 (не kit v2 / не `LongJob`): `agents/tnav_cluster.py`, `tnav-cluster-service/` (paramiko SSH, белый список команд без `rm`, pydantic `ClusterSettings`). Инструменты: `list_models`, `inspect_model`, `prepare_model_version`, `start_calculation` (`in_progress` + поток), `check_calculation`, `ask_engineer`. Lab — `mock_cluster/` (SSH-сервер + заглушка `tNavigator-con`); поле — `tnav-cluster.env`.
2. **10.2 Знания** — ✅ ревизия 55 карточки `cluster-model-layout` / `cluster-prepare-version` / `cluster-run-calculation` в `specialist_template` (пространство `cluster_ops` не заводили — долг). `route-cluster-calculation` r6: `topics` = `output_provides` агента (`model_version`, `run_status`, `run_results`) — карточка в срезе оркестратора только когда `tnav_cluster` включён в реестре.
3. **10.3 Live-слот** — 🟡 ревизия 55: слот `tnav_cluster_run` в `--live` 13/13 GREEN. Нет: четырёхагентная цепь Excel → Builder → Cluster; `agent_down_recovery` для кластера; отмена во время `waiting_agent`.
4. **10.4 Учёт трения** — kit `extra_health`; схема Activity: JS реестровый, Python `schema_view` получил узел `cluster`. Discovery манифеста/гейта по-прежнему руками (K11).

**Критерий фазы:** `done` в live с заглушкой; оркестратор JSON не правился; `--live` 13/13. Полный критерий «`git diff --stat` вне папки ≤ 3» не выполнен: агент селился в манифест/гейт/docs/compose (ожидаемо до K11).

**Границы:** реальный кластер, очереди, лицензии tNavigator — поле, не lab; разбор бинарных результатов (UNSMRY/UNRST) — следующий агент.

### Фаза 11 — Activity production ⬜

**Deliverables:** 11.1 B1 (намерение до вызова, single-flight); 11.2 B2 (часы stale по оркестратору); 11.3 B3 (кэш по генерации прокси); 11.4 B4 (`run_generation`); 11.5 B7 (ключ идемпотентности по payload); 11.6 B9/B10 (host по умолчанию для поля документирован, метрики без шторма); 11.7 UI U1–U6, U8 (cache-bust по хэшу, локальные шрифты/`marked`, счётчики и RU-подписи, `CASE-…` только в dev-mode, подсказка «ждём оркестратор» гаснет, «Агенты» для инженера, мелочи доступности).

**Критерий фазы:** тесты B1–B4/B7 в `test_cases_api`; UI работает при `network offline`; ни одной английской подписи вне dev-mode; `--live` 13/13.

### Фаза 12 — Гейт ≤ 20 мин, метрики, golden второго поколения ⬜

**Deliverables:** 12.1 T2 (`LIVE_WALL_BUDGET_S=1200` — красный при превышении; порядок слотов: параллельные, затем recovery; `--repeat 3` — ночной режим); 12.2 T5 полностью (`rag_empty`, `llm_truncated`, `kb_calls` в порогах); 12.3 Q5 (таймауты вниз по `llm_ms`); 12.4 U10 страница «Метрики»; 12.5 F2/6.5 (два HITL от разных агентов; `review_scope: agent`; rebind + commissioning в одной задаче; `REVISE` с INCLUDE/`.dev` — требует S3; `failed` → повтор → `done`); 12.6 S3/S7 live (INCLUDE-кейс, ops с датой).
...
**Критерий текущего гейта (ревизия 57).** Офлайн: regen drift 0, smokes по glob (18, включая `tnav-cluster-agent-smoke.js`), pytest-наборы включая tNav Cluster и demo, combat; live: 13/13, `mismatch_count: 0`, `warnings: 0`, `steps ≤ 6`, `tool_calls ≤ 4`, `llm_truncated: 0`, `rag_empty: 0`, без повторных HITL и `completion_review`; `excel_apply_dataset` `kb_calls ≥ 1`; commissioning шаг 0 ⊆ четырёх policy; golden_1 ≠ golden_2 `orch_rag_ids`; file-less demo не несёт сразу excel и schedule; `tnav_cluster_run` `waiting_agent` → `run_status=finished`; стена одиночного `--live` ≤ 20 мин (контроль, не критерий — T2). Prepare RAG без regex по постановке. Embeddings `baai/bge-m3` / v2. Полевой `*.env` кластера и шаблона — Python `load_service_env`, не CMD `for /f`.

---

## 5. Что остаётся детерминированным (явно)

- Parse/apply/emit SCHEDULE; блочный `/`; порядок keyword внутри DATES; рендер записей по каталогу схем (S2 переводит `apply_operations` сюда же); INCLUDE-граф и безопасность путей (S3).
- Инвентарь книг Excel (геометрия таблиц, заголовки, типы ячеек); разбор дат/чисел; `extract_*` по названным LLM колонкам.
- Retrieval: SQL веток, RRF, изоляция по `target_base`/типам/скоупу, гидратация, ревизии. Что искать — решает вызывающий (инвентарь/LLM), не regex по тексту.
- Оркестратор: лимиты шагов/ошибок, `Verify completion` из частей, guards по плану, слияние результата, `run_generation`.
- Kit: конверты ошибок, гейт машинности текста, `JobStore`/heartbeat/recover, потоковые бинари.
- Кластерный агент: разбор `.DATA`/INCLUDE, копирование версии модели, вызов CLI, опрос статуса. LLM — выбор модели/версии/профиля и вопросы инженеру.
- Гейт: пороги α2 + `rag_empty`/`llm_truncated`/`kb_calls`, семантическое сравнение `.INC`, бюджет стены.
...
|---|---|---|
| Цикл агента на HTTP (7.3) изменит поведение Qwen (порядок инструментов, повторы) | рост `steps`/`tool_calls`, новые HITL | Флаг `llm_transport` с откатом на одну ревизию; `--repeat 3` до/после; профили сэмплинга измеряются, не угадываются |
| Корпоративный endpoint игнорирует часть выключателей thinking | пустой `content`, `finish_reason=length` | Слать все три (Q2); `trace.llm` покажет `<think>`/усечение сразу; порог `llm_truncated: 0` |
| Оживший RAG (8.1) принесёт Builder'у лишние карточки и увеличит промпт | латентность, `tool_calls` | `summary` вместо полного текста (8.2), `max_cards` 4–6, полный текст только по `retrieve_knowledge` |
| Русский FTS (`russian` config) меняет ранжирование ALLCAPS-keyword'ов | `exact_hit` теряется | Два индекса (`russian` для прозы, `simple` для ALLCAPS), `exact_hit` boost сохраняется, smoke `WCONPROD` |
| Pydantic-модели изменят JSON (порядок ключей, `None`) | падение Format JS/`applyAgentResult` | `model_dump(exclude_none=True)` + фикстуры «до/после» байт-в-байт |
| Discovery спек подхватит черновик агента | лишний workflow в манифесте/Health | Спека `enabled=False` + `draft=True` исключается из манифеста; тест |
| `LongJob` на Windows (потоки vs процессы, блокировки файлов) | зависший job после рестарта | `filelock` уже в kit; `recover_jobs` с lease; тест на Windows-путях (pytest без Docker) |
| Кластерный агент упрётся в kit там, где demo не упирался | «часы» на правки kit | 10.4: каждая правка — строка долга с тестом, бриф останавливается после второго красного live |
| Live-гейт вырастет за 20 мин с новыми слотами | красный T2 | Параллельные пулы для независимых слотов; recovery последовательно; ночной `--repeat 3` |
| Безопасность Activity (B8) отложена | доступ без авторизации в LAN | Явное решение заказчика; фиксируется здесь, не в очереди |
...
### Ревизия 33 (2026-09-12) — удалён retired-контур n8n

Удалены `n8n/templates/retired/`, `n8n/workflows/retired/`, `n8n/tests/retired/`, `n8n/contracts/`, `n8n/data-tables/`, JS schedule-intake/pipeline (живой emit — FastAPI), support-заглушки кроме `demo-agent`, `context-seeder/`, неиспользуемые schema JSON. `generate_schedule_workflows.py` пишет только Knowledge Ingestion + Retrieval в `core/`. Lab import — 10 JSON (9 core + demo-agent). Legacy Activity `/v1/tasks*` и Excel `/api/v1` оставлены (F7 / pytest). `--live` не гоняли: оркестратор/агенты/emit не менялись.
...
### Ревизия 34 (2026-09-12) — удалён Activity `/v1/tasks*` (F7)

Удалены in-memory маршруты `/v1/tasks*` / `/v1/turns` / `/v1/sync` / `/v1/hydrate` / `/v1/demo/seed` и модули `durable.py`, `enrich.py`, `commissioning.py`. `main.py` — `/cases`, `/health`, `/ready`, `/v1/diagnostics`, `/v1/knowledge/*`, статика. Lab probe `lab_soft_redeploy.py` → `GET /cases`. Pytest Activity: живые проверки UI/health/CORS + регресс 404 на legacy. `--live` не гоняли: оркестратор/агенты/emit/`cases_api` не менялись.
...
### Ревизия 43 (2026-09-13) — архитектурный аудит всей системы, план переписан

Сделано (без правок кода): пять глубоких ревизий — Excel-агент (workflow + сервис + карточки), Schedule Builder (workflow + сервис + знания + движок), Activity (backend, лог, UI), kit/шаблон/спеки/генераторы, RAG (retrieval, ingest, корпус, UI знаний) — плюс разбор оркестратора (`generate_mas_orchestrator.py`, `mas_state_utils.py`, `mas_retrieval_client.py`), лога `CASE-6aa6b6fc-3280a9` и 30 последних executions `MAS — Knowledge Retrieval` через lab REST. План переписан: §0 диагноз с доказательствами, §2 — новый инвентарь (O24–O30, R1–R14, X1–X9, S1–S12, B1–B12, U1–U10, K8–K18, Q1–Q5, T2–T5), §3 — принципы 12–16, §4 — фазы 7–12 с критериями, старые §2/§4 — Приложения A/B без изменений.

Live-доказательства (lab, 2026-09-13 14:50–14:54):

- `MAS — Knowledge Retrieval` #228535, #228336: `target_base=schedule_mvp`, `keyword_families=[]` → `status=abstain`, `findings=[SCHEMA_KEYWORD_SCOPE_REQUIRED]`, 0 результатов. `Agent — Schedule Builder` #228529, #228330, #228230, #228121: `rag.status=unavailable`, `cards=[]`, `planner_input` 39 800–40 343 символов. **Builder ни разу не видел базу знаний на commissioning.**
- `Orchestrator — MAS` #228559, #228501, #228497, #228414, #228413, #228363, #228320, #228288: `rag.status=ready`, карточки всегда `route-mas-thin-orchestrator`, `route-excel-extractor`, `route-schedule-builder` (из 8 найденных retrieval'ом; остальные отсечены порогом 45 %). `planner_input` 9 640–13 454 символов.
- `Agent — Excel Extractor` #228440, #228425, #228297: все 5 карточек `excel_protocol` каждый вызов (тег `протокол` статичен), три описывают снятый протокол.
- Лог `CASE-6aa6b6fc-3280a9`: нет `trace.rag`, нет токенов на успешных решениях, нет LLM-ходов агентов, нет длительностей между событиями.
- Гейт на момент аудита: офлайн GREEN (regen 1 с, 16 smokes, 5 pytest-наборов, combat); live 12/12 GREEN ревизии 42 (`CASE-6aa6b6fc-*`, `steps` 3–4, `warnings: 0`, `tool_calls` 1–4, 63–115 с на кейс).

Решения ревизии:

- Thinking у агентов закрывается **не** попыткой настроить `lmChatOpenAi`, а переносом цикла агента на HTTP `/chat/completions` (Фаза 7.3) — тот же транспорт, что у Decision; выключатели thinking — все три (`reasoning.enabled`, `enable_thinking`, `chat_template_kwargs.enable_thinking`); Qwen3.6 не поддерживает soft switch `/no_think` (model card).
- Порядок фаз: видеть (7) → знания (8) → kit (9) → кластерный агент (10) → Activity production (11) → гейт/метрики/golden (12). Обоснование — §2.8.
- Требование заказчика «полный прогон ≤ 20 мин» закрепляется критерием T2 (Фаза 12); до этого — контроль стены.
- Безопасность Activity (B8) — вне объёма по решению заказчика; зафиксирована в §6.
...
### Ревизия 54 (2026-09-18) — оркестратор и знания (8.6)

Сделано: O24 — `buildRetrievalQuery` без файлов/журнала; `planRetrievalTopics` = роли приложенных файлов ∪ открытого плана ∪ `completion` (без `absent_agent`); `SELECTORS.orchestrator` top_k 20 / text_limit 1500; `compactRetrievalCards` на `orchestrator_routing` при непустых `topics` оставляет карточку только при overlap тегов (2-ветка 45 % без тега отбрасывается). O28 — Verify: карточки `topics: completion`. O29 — секции `planner_input` Цель→План→Журнал→Данные агентов→Открытые вопросы→Реестр→Политика. O30 — SYSTEM: политика важнее догадки. HITL-карточка `topics` += `completion`. Харнесс: `orch_rag_ids`, шаг 0 ⊆ policy, plan_policy по приложенным ролям, distinct golden_1/golden_2, demo не оба excel+schedule. Бриф `briefs/2026-09-18-8.6-orchestrator-knowledge.md`.

Live 1 (`--only live` после полного import, overlap ещё не было): RED golden_2 `CASE-6aacea63-31c63b` (план без excel_extractor, RAG всё ещё excel+thin+schedule); demo `CASE-6aaceb1c-1e3cbe` обе карточки. Live 2 (overlap-only, 488 с): commissioning 6/6 GREEN, golden_2 без excel; chain `CASE-6aacef32-374aa4` `rag_empty=1` на шаге demo (файловые теги снимались с `done` пунктов); down `CASE-6aacefed-0cd4ec` гонка `status_message` на первом `failed`. Live 3 (файловые теги остаются, 455 с, полный import): 12/12 GREEN.

Live 3: golden_1 `CASE-6aacf221-68489c` orch excel+thin+schedule steps 3; golden_2 `CASE-6aacf221-9eafe8` thin+schedule steps 2; combat_0 `CASE-6aacf221-4d6e44`; combat_1 `CASE-6aacf221-013609`; combat_2 `CASE-6aacf221-63e254`; combat_3 `CASE-6aacf221-ec0189` steps 4 `tool_calls: 4`; demo `CASE-6aacf2bb-37b929` `orch_rag_ids: []`; chain `CASE-6aacf2bb-eb77f8` план 3/3 `rag_empty: 0`; extract `CASE-6aacf2bb-5b4b69`; apply `CASE-6aacf2d7-f092ea` `kb_calls: 1`; gap `CASE-6aacf322-d80614`; down `CASE-6aacf344-3f8688`; step_limit `CASE-6aacf38c-fd7603`; rework `CASE-6aacf38c-927153`.

Гейт: офлайн GREEN (17 smokes, 5 pytest-наборов, combat); live 12/12 GREEN.

Не сделано: 8.7; удаление `n8n_agent` (ревизия 57); inventory `rag_inventory_incomplete` при 61/61 в v2 (ложный missing id, 8.7); file-less demo `rag_empty: 2` / пустой orch срез (routing-карточки не пересекаются с `well_count`/`wells`); трекинг `run_live_*.py` вне gitignore.

### Ревизия 55 (2026-09-18) — tNav Cluster Agent (10.1 / 10.2 / слот 10.3)

Сделано по запросу заказчика, раньше kit v2. Сервис `tnav-cluster-service` (pydantic v2, paramiko, белый список команд без `rm`, корень `TNAV_CLUSTER_ROOT`, команда `{model}` в `TNAV_CLI_COMMAND`). Мок ГД-кластера: дерево моделей, фейковый `tNavigator-con`, SSH-сервер; lab Compose — сервис ходит к моку по SSH. Спека `agents/tnav_cluster.py`, workflow `Agent — tNav Cluster Agent`, `tnav_cluster_url` в Runtime Config, строка реестра `enabled=false`. Карточки `cluster-*` в `specialist_template`; `route-cluster-calculation` r6 (`topics` = `output_provides`, слот кластера в `--live` сериализован после demo∥datasets — иначе карточка текла в step-0, `CASE-6aad611d-4df69e`). Схема Activity: JS по реестру; Python `schema_view` — узел `cluster`. Полевой путь: `docs.md` §8 и `tnav-cluster-service/README.md` (куда писать адрес, логин, пароль, корень, команду). Live-слот `tnav_cluster_run` в `--live` (13-й). `field_check` пропускает кластер, если он не запущен.

Live RED → GREEN. Мок SSH закрывал сессию после первой exec (`CASE-6aad611c-732bd3`, `open_session` EOFError); SFTP-абсолютный путь и nohup-консоль как `.err` при отсутствии `ps` в slim (`CASE-6aad780d-952112`); после `start_calculation` Qwen крутила `check_calculation` пока мок заканчивал расчёт (`CASE-6aad7bd4-871c26`, два `agent.result`, не было `waiting_agent`). Фикс: много-exec SSH + `_real` для абсолютных путей; `process_alive` через `/proc/<pid>`; `.lock` важнее баннера консоли; `result_guard` на инструментах после `in_progress`.

Live 13/13 GREEN (`--only live` 596 с после офлайн GREEN той же сессии): golden_1 `CASE-6aad7d24-0f6aaf`; golden_2 `CASE-6aad7d24-22836f`; combat_0 `CASE-6aad7d24-dd5c1d`; combat_1 `CASE-6aad7d24-ce61da`; combat_2 `CASE-6aad7d24-2f970c`; combat_3 `CASE-6aad7d24-7586bd`; demo `CASE-6aad7db8-741ae8`; chain `CASE-6aad7db8-f8355f`; extract `CASE-6aad7db8-6bad88`; apply `CASE-6aad7df0-6ff3c0` `kb_calls: 1`; gap `CASE-6aad7e5f-a2821e`; cluster `CASE-6aad7e84-755c20` `waiting_agent` → `run_status=finished` steps 2 tool_calls 3; down `CASE-6aad7ec9-0bdcf1`; step_limit `CASE-6aad7f09-106f2c`; rework `CASE-6aad7f09-f73af9`.

Гейт: офлайн GREEN (regen/smokes/pytest/combat); live 13/13 GREEN.

Не сделано: четырёхагентная цепь; отмена расчёта; `cluster_ops`; `LongJob`; удаление `n8n_agent` (ревизия 57); 8.7; Excel/Schedule `start-windows.bat` ещё `for /f`.

### Ревизия 56 (2026-09-19) — полевой `*.env` как у Activity (kit + кластер + шаблон)

Прод: CMD `for /f` в `tnav-cluster-service/start-windows.bat` портил UTF-8 BOM и пароли/`TNAV_CLI_COMMAND` с `=`; Python `load_dotenv(..., override=False)` кривые ключи не перебивал. Канон Activity: bat не парсит файл, `python -m app`, `utf-8-sig`.

Сделано: `mas_agent_kit.env.load_service_env`; `/health.env_files`; кластер и `agents-template/demo_agent` — `app/__init__.py` + `app/__main__.py` + `start-windows.bat` / `start-linux.sh` без `for /f`/`source`. `python-dotenv` в requirements kit и шаблона. Тесты: BOM/`=` в kit, пароль с `=` и bind в cluster, `test_service_shape` на шаблон и кластер.

Не сделано: Excel Tools и Schedule Builder всё ещё парсят env через CMD (тот же дефект); `--live` на этой ревизии не гоняли (оркестратор/агентный граф не менялись).

### Ревизия 57 (2026-09-19) — сверка docs / плана / правил с кодом

Код не меняли. Устаревшие формулировки: гейт «17 smokes / 5 pytest / 12 live» в §1; критерий §4 помечен ревизией 55; `docs.md` «после 6.2» и «пустой RAG Builder’а на датах ввода ожидаем»; правило Excel с ключом `error` в конверте; в полевой таблице привязок не было второй RAG-ноды агента (`Retrieve knowledge`).

Сделано: шапка/§2.7/§3/§4 = ревизия 57; §1 и критерий гейта — 18 smokes, 6 pytest-наборов, 13 live; Фаза 8 🟡 (остаток 8.7); B12/K14/T4 помечены по факту; T6 — Excel/Schedule `for /f`. `docs.md`: две RAG-ноды, env bat, лог `rag_empty`, очередь без «6.2». `AGENTS.md`: `--repeat` не троить 13 слотов. `.cursor/rules/excel-agent-tools.mdc` — конверт kit.

Не сделано: 8.7; удаление `n8n_agent` (ревизия 58); T6; `--live` на сверке не гоняли.

### Ревизия 58 (2026-09-19) — 8.7 ingest и UI знаний

Сделано: skip ingest только если чанки в v2 и текст/`content_hash` совпали; тот же id/rev с другим текстом → `CONTENT_HASH_CHANGED_BUMP_REVISION`; `purge_superseded`; skipped id считаются found. Activity: `GET /v1/knowledge/{base}?q=&tag=`, `document_count`, `GET …/revisions` (`list_knowledge_revisions` в прокси), ingest одной карточки, RU-подписи вкладок, schema/examples. UI: серверный поиск, клик по тегу, локальный `marked` + IBM Plex в `static/vendor/` (без Google Fonts/jsDelivr). `--live` knowledge probe. Кластерный слот: промпт «только function calling» (`CASE-6aae41e4-a48ba9` писала `inspect_model("…")` в content); харнесс сбрасывает leftover `SEVER_*.data` (`version_not_created`).

Офлайн: regen sync, 18 smokes, pytest kit 51 / Activity 135 / Builder 65 / Excel 89 / tNav 26 / demo 7, combat. Live 13/13 GREEN (518 с, `--hot`): commissioning `mismatch_count: 0`, `excel_apply_dataset` `kb_calls: 1` `tool_calls: 3`, cluster `waiting_agent` → `done` `tool_calls: 3`.

Не сделано: удаление `n8n_agent` (ревизия 59); T6 Excel/Schedule `for /f`; хвост 10.3 (четырёхагентная цепь, cancel, `cluster_ops`, LongJob). Qwen на apply иногда зовёт `list_records` сверх 4 tools (`CASE-6aae44b3-936188`) — повтор слота GREEN, в очередь не ставим.
...
1. **Ревизия 59** — удалить откат `llm_transport=n8n_agent` (двойной граф).
2. **9.1 – 9.4** — pydantic-контракты, двойники из модели, чеклист-тест, discovery спек и производные (манифест/Health/гейт/zip).
3. **9.5 – 9.8** — `LongJob`, потоковые бинари, агенты на общей форме (Builder `packet()`, Excel без снятых инструментов, один кастер дат, `repeat_guard`), рецепт нового агента; перенос кластерного фона на `LongJob`.
4. **10.3 хвост** — четырёхагентная цепь Excel → Builder → Cluster; отмена `waiting_agent`; пространство `cluster_ops`.
5. **11** — Activity production (B1–B4, B7, B10; U1–U6, U8).
6. **12** — стена `--live` ≤ 1200 с как критерий, страница «Метрики», golden второго поколения (F2), таймауты вниз.

Не очередь: B8 (безопасность) — решение заказчика; S12 — по запросу инженера; судью «красиво ли написано» не добавлять.

---

## Приложение A. Инвентарь долга ревизий 1–42 (как был на ревизию 42; коды O1–O23, H1–H11, A1–A17, K1–K7, L1–L2, T1, R1–R5*, F1–F10, E1–E6)

> Историческая часть, не переписывается. Активная очередь — §2.8 и §7 «Ближайшие шаги»; активный инвентарь — §2. Коды `R1–R5` этого приложения (результат кейса как один файл) не совпадают с серией R (retrieval) в §2.2.
...
## Приложение B. Фазы 0–6 и альфа — критерии как были (ревизия 42)

> Историческая часть, не переписывается. Новые фазы 7–12 — §4.
