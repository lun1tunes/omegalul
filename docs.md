...
Запрос оркестратора — текст цели, заголовков пунктов плана и журнала (без regex-тегов и имён файлов); теговая ветка Retrieval — по `keyword_families` / `topics` / `task_patterns` карточек. Источник карточек — `n8n/rag/excel-agent-operating-guide.documents.json` (блок `injection_template` ingest игнорирует).
