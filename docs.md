...
Система по задаче инженера-гидродинамика собирает или правит `SCHEDULE` (`.INC`) для tNavigator/ECLIPSE: читает Excel с датами ввода и параметрами скважин, задаёт вопросы по-русски, детерминированно применяет изменения к исходному файлу schedule и отдаёт новый `.INC` и diff. SCHEDULE — первый агентный контур; архитектура рассчитана на добавление агентов (расчёты, кластер, результаты) без правки оркестратора (§6).
...
- **План** `state.plan[{id, title, agent_id?, status, depends_on?, note?}]` — декомпозиция **цели инженера** на результаты (1–5 пунктов), не перечень всего, что агенты умеют (`output_provides` — каталог возможностей, не чек-лист). LLM ведёт план через `plan_update` (пункт без `id` отклоняется и считается в `orchestrator.decision.payload.plan.rejected`; `agent_id` принимается и по title из реестра). Статусы `pending | active | done | blocked | dropped`; `pending → active → done` проставляет сам оркестратор по действиям (`call_agent` → `active`, агент вернул `completed` → его `active`-пункт `done`; если в `data` уже есть все ключи `output_provides` из реестра — закрываются и остальные открытые пункты этого агента, `CASE-6aa029de-c5e23d` — защита от цикла, не требование «достать все ключи»), LLM меняет вручную только `blocked` / `dropped` / пункты без агента. Decision LLM и `Verify completion` видят план блоком «План задачи» рядом с журналом; проверка завершения берёт обязательные части из **цели**, пункт плана сверх цели не дырка (`CASE-6aa052eb-263a16`). Заголовки пунктов идут в текст RAG-запроса, а роли агентов открытых пунктов (`input_required` / `output_provides` из реестра) — в теги `filters.topics` того же запроса (`planRetrievalTopics`). `finish` при открытом пункте (`pending` / `active` / `blocked`) обрабатывается как отказ проверки завершения (см. ниже) — закрыть пункт можно в том же ответе, что и `finish`. Решение `finish` пишется в ленту как `orchestrator.decision` (с планом и счётчиками `plan_done/plan_total`) непосредственно перед `case.finished` — в чате это одна строка, в логе разработчика — отдельный шаг. Итоговый план — в обоих событиях; Activity показывает его блоком «План · N из M» под постановкой задачи (`feed.plan`).
- **Решение** — Structured Output с обязательным `progress {goal_satisfied, evidence, missing, is_repeating}` и действием `call_agent {agent_id, handoff_message, expected_output?}` / `ask_user {question, options}` / `finish {summary_for_human}`. В `agent_task.inputs` оркестратор кладёт только ссылки (`activity_base_url`, `schedule_root`, `artifact_ids`, `data_refs`, `rework_reason`, `unlisted_wells_policy`) и, если Decision LLM назвал форму набора **нужную цели**, `expected_output` (`datasets` — санитизированные имена/поля, `consumers` — `input_schema.data` открытых пунктов плана). Не копировать в `datasets` все ключи `output_provides`. Факты-значения от LLM в задачу не копируются — поручение агенту передаёт `handoff_message`.
- **Предохранители в `Parse decision`** (каждый пишет `guard` в `orchestrator.decision.payload`): ответ человека должен дойти до агента, который спросил (`answer_not_applied` — `finish` невозможен, пока тот не вернул `completed`); повторное делегирование агенту с `completed` — один раз и только с `rework_reason`, дальше review-гейт с кнопками «Принять результат / Нужна доработка»; вызов того же агента для **другого открытого пункта плана** (`action.task_id` = id пункта) — не повтор, а новая работа (`planNamesNewWork`; `CASE-6aa00e09-c6d71f`: модель разложила Excel на два пункта, второй вызов ушёл в review, а «принять» завершило кейс без Builder'а), но если в `data` уже есть все `output_provides`, оставшиеся пункты этого агента закрываются и второй handoff — `repeat_review` (`CASE-6aa029de-c5e23d`). Что означает «принять» в review-гейте, определяет план: если в нём остались пункты агентов, которые ещё не работали, — принимается результат одного агента и работа продолжается (`review_scope: agent`, кнопка «Принять результат и продолжить»; повторный вызов принятого агента — снова review, `finish` проверяется по плану); иначе — итог задачи (`review_scope: case`, `human_accepted` → `finish`). Флагу `goal_satisfied` при первом делегировании не верят (`goal_flag_ignored`); ответ человека сбрасывает stall. Нечитаемое или неизвестное действие LLM (`call_agent`/`ask_user`/`continue`/`finish`) — `decision_unparsed`: один шаг `continue` с записью в журнал «решение оркестратора не разобрано», второй подряд — `case.failed` по-русски, **не** вопрос инженеру. Сырой ответ модели — в `orchestrator.decision.payload.decision`.
- **Проверенное завершение:** `finish` не принимается на слово — второй скептический LLM-проход `Verify completion` требует для каждой **части цели инженера** запись журнала `completed` (вложения инженера частью цели не считаются; пункт плана сверх цели не обязателен). Отказ проверки или открытый пункт плана → ещё один шаг с указанием пробела (`completion_unverified`); второй отказ → review-гейт прозой (`completion_review`); человек принял — `finish`. `case.finished.payload.completion_verified` показывает, прошёл ли итог проверку.
- **Ошибка агента** (`agent.failed`: узел не выполнился, агент не привязан, HTTP 5xx у LLM) — запись в журнал и новое решение; лимит ошибок и `max_steps` — из Runtime Config. По достижении `max_steps` с готовым результатом — review-гейт, без результата — честный `failed`. Недоступный сервис агента (`open_session` без JSON `{ok:…}` после ретраев) — `failed` + `issues[{code: service_unreachable}]`, сразу `case.failed`, не ложный HITL «приложите файл». Decision LLM и Verify completion при 503 модели повторяют вызов (3×2 с) и при окончательной ошибке не роняют execution (`onError` continue).
...
|---|---|---|
| Оркестратор (`Orchestrator — MAS`) | `orchestrator.decision` (в т.ч. `guard`), `agent.handoff`, `hitl.request`, `hitl.answered`, `orchestrator.resume`, `case.finished/failed/cancelled`, `agent.failed` | усечённая копия решения LLM (`decision`), переданная агенту `agent_task`, вопрос и варианты, ответ инженера, `execution_id`/`workflow_id` |
...
Запрос оркестратора — текст цели, заголовков пунктов плана и журнала (без regex-тегов и имён файлов); теги `filters.topics` — роли агентов открытых пунктов плана из реестра (`input_required` / `output_provides`, например `excel`, `facts`, `schedule_source`, `schedule_out`), поэтому карточки `orchestrator_routing` для маршрутов несут те же слова в `topics`; на первом шаге (плана ещё нет) теги пусты и работают только lexical/semantic ветки. Источник карточек — `n8n/rag/excel-agent-operating-guide.documents.json` (блок `injection_template` ingest игнорирует).
...
- Excel читает только Excel Tools (`:8000`); Schedule Builder получает факты из результатов агента, читавшего Excel (`state.agents[*].data.facts` / `new_wells` / именованные `kind:dataset`), и из ответов инженера — сам `.xlsx` не открывает.
...
- Удаление скважин — только по явному решению инженера (`unlisted_wells_policy`: «оставить как в исходном файле» / «убрать из прогноза»); wildcard-записи (`'*'`) не трогаются.
- **INCLUDE:** позиция относительно `DATES` сохраняется; текст INCLUDE не переписывается; пути как в исходном файле (в т.ч. `'../../INCLUDE/…'`); тело подтягивается, если файл передан (`schedule_files`), иначе вызов остаётся (`KEEP`); URL и абсолютные пути запрещены, выход `..` за корень пакета — unsafe. Несколько файлов — `schedule_files` + `schedule_root`.
...
| «Сервис агента не отвечает» / кейс `failed` сразу после старта | сервис на Windows не запущен или адрес в Runtime Config не тот; в логе `issues.code = service_unreachable`. Поднять сервис → в ленте **«Перезапустить с теми же файлами»**. Не путать с вопросом «приложите Excel/SCHEDULE» — файлы уже есть |
| Баннер «Сбой» с кодом/`failed` | не должен; в ленте русская `status_message`. «Продолжить» — если оркестратор завис (`running`/`waiting_agent` без событий > 2 мин); «Закрыть задачу» — статус `cancelled` |
...
Один вердикт: `python3 scripts/mas_gate.py` — регенерация всех workflow с проверкой дрейфа, 16 smokes, пять pytest-наборов (kit, Activity, Schedule Builder, Excel Tools, demo agent), offline combat; `--live` добавляет `lab_soft_redeploy.py` (импорт workflows, SQL, рестарт Python-сервисов; падает, если сервис не поднялся) и **12 живых кейсов** через Activity: шесть `run_live_five.py` (golden 1–2, combat 0–3; `done`, `mismatch_count: 0`, без повторных HITL / циклов) + `run_live_demo_agent.py` — `demo_agent_long_job`, `three_agent_chain` (Excel → Builder → demo; план из трёх пунктов `done`, `warnings: 0`) + три recovery (`agent_down_recovery`, `rework_round`, `step_limit_review`) + `run_live_excel_datasets.py`; `--cases demo_agent|three_agent_chain|excel_datasets|agent_down_recovery|…` гонит один. После зелёного офлайна: `python3 scripts/mas_gate.py --only live`. Разбор кейса: `python3 scripts/mas_trace_case.py CASE-… [--n8n --node "Parse decision"]` — лента, state (`agents.<id>`, журнал, HITL), аудит машинного текста, подсказки, n8n executions; то же без консоли — `http://127.0.0.1:8200/cases/CASE-…/log` и вкладка «Лог» (§1.5; в lab `N8N_PUBLIC_URL=http://localhost:5678` в compose, чтобы ссылки на executions открывались из браузера). Правила работы — `AGENTS.md`, `.cursor/rules/`, `.cursor/skills/`.
...
Файлы и данные предыдущих агентов агент берёт сам (`CasePacket` в kit'е): карточки — `GET {activity_base_url}/cases/{case_id}/state` (`artifacts`, `agents.<id>.data`), содержимое — `GET /cases/{case_id}/artifacts/{artifact_id}`, факты предыдущих агентов — `packet.upstream_data()`, именованные таблицы — `packet.dataset(name)`. Если оркестратор передал `inputs.expected_output.datasets`, агент, читающий Excel, извлекает каждый набор (`extract_table` / `extract_commissioning` / `extract_well_parameters`); без этого ключа commissioning-вызовы не меняются.
...
```json
{
  ...
  "data": {"facts": [...], "well_events": {"kind": "dataset", "name": "well_events", "fields": […], "rows": […]}, "…": "всё, что понадобится следующему агенту (ключи = output_provides или имя набора)"},
  ...
  "requests": [{"question_id": "Q-topic", "question": "русская фраза", "options": [{"value": "keep", "label": "Оставить как в исходном файле"}], "accepts": {"free_text": true, "files": ["xlsx"]}}],
...
| `POST /agent-tools/{tool}` `{session_id, …args}` | один инструмент под файловой блокировкой сессии; успех — `{ok: true, …}`; **ошибка аргументов — для LLM**: `raise ToolError("column_not_found", "…что исправить…", available_columns=[…])` → `{ok: false, code: "column_not_found", message, …}` (ключ `code`, не `error`: n8n 2.30.8 считает item с `json.error` упавшим запуском ноды и при `retryOnFail` шлёт тот же HTTP-вызов повторно) |
...
| (любой инструмент, зафиксировавший результат) | `agent.store_result(state, result)` сохраняет `agent_result` для оркестратора и возвращает его **вид для модели**: тот же результат плюс `next_step` — русская строка «кто действует дальше» (`RESULT_NEXT_STEP` в kit'е: для `needs_input` — «вопрос уже поставлен, `ask_engineer` не нужен, заверши ответ»). Возвращать инструменту именно этот вид; повторный `ask_engineer` после этого отбивается `result_already_stored` |
