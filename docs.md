...
Один вердикт: `python3 scripts/mas_gate.py` — регенерация всех workflow с проверкой дрейфа, 16 smokes, пять pytest-наборов (kit, Activity, Schedule Builder, Excel Tools, demo agent), offline combat; `--live` добавляет `lab_soft_redeploy.py` (импорт workflows, SQL, рестарт Python-сервисов; падает, если сервис не поднялся) и живые кейсы через Activity: шесть `run_live_five.py` (golden 1–2, combat 0–3; `done`, `mismatch_count: 0`, без повторных HITL / review / циклов) + два `run_live_demo_agent.py` — `demo_agent_long_job` (агент-шаблон: `in_progress` → `waiting_agent` → `resume source=agent` → `done`) и `three_agent_chain` (combat 1 + задача для demo-агента: Excel → Builder → demo; проверяет порядок handoff'ов, `state.plan` из трёх пунктов `done` с `agent_id` фактической цепи, `.INC` семантически равен golden, в логе `warnings: 0`) + `run_live_excel_datasets.py` (`excel_datasets`: книга мероприятий без `.INC`, `extract_table` → `kind:dataset` в `state.agents.excel_extractor.data`, не только `facts`); `--cases demo_agent|three_agent_chain|excel_datasets` гонит один. Разбор кейса: `python3 scripts/mas_trace_case.py CASE-… [--n8n --node "Parse decision"]` — лента, state (`agents.<id>`, журнал, HITL), аудит машинного текста, подсказки, n8n executions; то же без консоли — `http://127.0.0.1:8200/cases/CASE-…/log` и вкладка «Лог» (§1.5; в lab `N8N_PUBLIC_URL=http://localhost:5678` в compose, чтобы ссылки на executions открывались из браузера). Правила работы — `AGENTS.md`, `.cursor/rules/`, `.cursor/skills/`.
