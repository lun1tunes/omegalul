...
from mas_agent_kit import ActivityClient
...
def _quiet_activity(monkeypatch, tmp_path):
    ...
    # Every Activity line goes through the kit client; capture instead of posting.
...
def test_open_session_without_workbook_asks_in_prose_and_writes_no_schedule() -> None:
    opened = agent.open_session(_task("Extract wells"))
...
def test_extract_commissioning_is_deterministic_and_summary_is_prose() -> None:
    ...
    kinds = [kind for kind, _ in agent_module._SEEN_EVENTS]  # type: ignore[attr-defined]
    assert "agent.accepted" in kinds
    assert "agent.result" not in kinds, "the orchestrator emits the single agent.result"
    assert any(msg.startswith("Даты ввода:") for _, msg in agent_module._SEEN_EVENTS)  # type: ignore[attr-defined]
...
def test_two_workbooks_become_one_session_with_per_file_inventory(monkeypatch) -> None:
    ...
    opened = agent.open_session(_task("даты ввода и новые скважины"))
    assert opened["ok"] is True
...
def test_extract_well_parameters_passes_unmapped_columns_under_their_headers() -> None:
    if not PARAMS_XLSX.is_file():
        pytest.skip("combat fixtures missing")
    opened = agent.open_session(_task("новые скважины", excel_path=str(PARAMS_XLSX)))
    state = load_state(opened["session_id"])
    table_id = opened["inspect"]["tables"][0]["table_id"]
...
def test_repeated_extraction_attempts_are_bounded() -> None:
    ...
    state = load_state(opened["session_id"])
    table_id = opened["inspect"]["tables"][0]["table_id"]
    for _ in range(agent_tools.MAX_TOOL_REPEATS):
...
def test_excel_cards_collect_every_workbook_of_the_case(monkeypatch) -> None:
    ...
    monkeypatch.setattr(ActivityClient, "case_state", lambda self: {"artifacts": {"excel": {"artifact_id": "excel_1", "filename": "params.xlsx", "role": "excel"}}})
    cards = agent.excel_cards({"case_id": "CASE-9", "inputs": {"activity_base_url": "http://mas-activity:8200", "artifact_ids": ["excel_1"]}})
...
def test_date_normalisation_accepts_engineering_formats() -> None:
    assert agent_tools._as_date("2020-02-23T00:00:00") == ("2020-02-23", True)
    assert agent_tools._as_date("23.02.2020") == ("2020-02-23", True)
    assert agent_tools._as_date("23 FEB 2020") == ("23 FEB 2020", True)
    assert agent_tools._as_date("Примечание") == ("Примечание", False)
    assert agent_tools._as_well(1601.0) == "1601"
    assert agent_tools._as_well("1601.0") == "1601"
